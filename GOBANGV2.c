...
void WelcomeInfo()
{
    ...
    gotoxy(12,6);
    printf("1.You can use the up, down, right and left keys to move the chessman,");
    gotoxy(12,8);
    printf(" and you can press the Space key to enter after you move it!");
    gotoxy(12,10);
    printf("2.You can use the Esc key to exit the game too!");
    gotoxy(12,12);
            ...
            if(step_x>=1&&step_x<=18&&step_y>=1&&step_y<=18)
            {
                ...
                if(status[step_x][step_y]==0)
                {
                    ...
                    if(ResultCheck(step_x, step_y)==1)
                    {
                        ...
                        setcolor(15);
                        settextstyle(3,0,4);
                        /*如果是player1获胜，则显示获胜信息*/
                        if(flag==1)
                        {
                            /*无衬笔画字体，水平放大5倍*/
                            outtextxy(20, 120, "Player1 win the game!");
                        }
                        /*如果是player2获胜，则显示获胜信息*/
                        if(flag==2)
                        {
                            /*无衬笔画字体，水平放大5倍*/
...
int main()
{
    ...
    {
        /*根据错误代码输出错误信息*/
        printf("\nNotice: Error occured when graphics initialization:%s\n", grapherrormsg(errorcode));
        printf("Press any key to quit!");
        getch();
        exit(1);
    }
    /*设置flag初始值，默认值是play1先行*/
    flag=1;
    /*画棋盘*/
    DrawBoard();
    ShowMessage();
